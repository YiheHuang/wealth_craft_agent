...
/// <summary>
/// 财务报告分析插件。
/// 为 LLM Agent 提供财务指标查询和盈利能力分析工具。
/// 包含 PE/PB/ROE/ROA/毛利率/净利率/增长率/负债率等核心财务维度的分析功能。
/// </summary>
public class FinancialReportPlugin
{
    ...
    /// <summary>
    /// 获取公司的核心财务指标，包含估值、盈利、成长和负债全维度。
    /// 缓存时间 30 分钟（财务数据更新频率低）。
    /// </summary>
        ...
        [Description("股票代码，例如600519")] string symbol)
    {
        ...
        var result = await _workingMemory.GetOrSetAsync(cacheKey,
            ...
            TimeSpan.FromMinutes(30)); // 财务数据变化慢，长缓存
